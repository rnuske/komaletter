...
Mail merge is just a variant of R Markdown's parametrized reports when using `komaletter`. The process is similar to mail merge in traditional word processing applications:
  ...
  * merge template letter and recipient's information
...
Since the address and the letter opening are defined in the YAML metadata. The corresponding parameters must be declared and after that put to use within the YAML metadata. Parameter values are accessed in R Code. In the YAML metadata header this means inline code snippets `` `r knitr::inline_expr("expression")` ``. Since backticks `` ` `` are 'reserved indicators' in YAML, the code snippet usually needs to be wrapped in quotes. 
...
As explained above, the address is a YAML sequence and sequences can not be enclosed in quotes. To access the parameter in the main YAML metadata header the inline R Code snippet has to be wrapped in quotes and R expressions wrapped in quotes evaluates to a value enclosed in quotes.
...
## Further Information
  * R Markdown
    * [Article 'Parametrized Reports' on RStudio's R Markdown website](https://rmarkdown.rstudio.com/developer_parameterized_reports)
    * [Chapter 'Parametrized Reports' in the book 'R Markdown: The Definitive Guide'](https://bookdown.org/yihui/rmarkdown/parameterized-reports.html)
    * [Lesson 6 'Parameters' in 'Get Started'-Course on RStudio's R Markdown website](https://rmarkdown.rstudio.com/lesson-6.html)
    
  * YAML 
    * [YAML Reference card](http://yaml.org/refcard.html) (Scalar indicators, Escape codes, Misc. Indicators for reserved indicators)
    * [YAML Specification](http://yaml.org/spec/current.html)(Syntax, Indicator Characters: reserved characters, Escape Sequences, Scalar Styles, Sequence Styles)
    * [YAML block sequence](http://yaml.org/spec/current.html#id2543032)
    * [YAML flow sequence](http://yaml.org/spec/current.html#id2542413)
